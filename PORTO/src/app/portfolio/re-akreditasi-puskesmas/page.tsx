...
export default function ReAkreditasiPuskesmasDetail() {
  const { isDark, toggleTheme } = useTheme()

  const projectDetails = {
    title: 'Re-Akreditasi Puskesmas',
    subtitle: 'Sistem manajemen dokumen dan monitoring proses re-akreditasi fasilitas kesehatan',
    duration: 'September - Oktober 2023',
    location: 'Bekasi, Jawa Barat',
    role: '',
    status: 'Completed',
    category: 'Manajemen Kualitas Layanan Kesehatan'
  }

  const features = [
    'Manajemen dokumen akreditasi terpusat',
    'Tracking progress persyaratan standar',
    'Reminder dan notifikasi deadline',
    'Digital evidence collection system',
    'Generate laporan akreditasi otomatis',
    'Collaboration tools untuk tim akreditasi',
    'Audit trail dan version control',
    'Mobile access untuk field verification'
  ]

  const technologies = [
    {
      name: 'Document Management',
      items: ['PDF Tools', 'Image Processing', 'Drive Storage', 'Microsoft Office Word']
    }
  ]
  ...
  const achievements = [
    {
      ...
      description: 'Berhasil meraih predikat Akreditasi Paripurna'
    },
    {
      icon: <Zap className="w-5 h-5" />,
      title: 'Tingkat Kepatuhan',
      description: 'Mencapai tingkat kepatuhan standar akreditasi menjadi 95%'
    },
    {
      icon: <Target className="w-5 h-5" />,
      title: 'Pengurangan Penggunaan Kertas',
      description: 'Mengurangi penggunaan kertas hingga 50% dengan sistem digital'
    }
  ]

  const challenges = [
    {
      title: 'Kompleksitas Standar Akreditasi',
      description: 'Mengimplementasikan sistem yang dapat mengakomodasi ratusan elemen penilaian akreditasi dengan berbagai kategori dan sub-kriteria yang kompleks.'
    },
    {
      title: 'Manajemen Dokumen Digital',
      description: 'Mengembangkan sistem manajemen dokumen yang aman, terstruktur, dan mudah diakses untuk berbagai jenis bukti fisik dan digital.'
    },
    {
      title: 'Koordinasi Multi-Departemen',
      description: 'Membangun platform kolaborasi yang memungkinkan koordinasi efektif antar berbagai departemen dan unit dalam proses akreditasi.'
    }
  ]
              ...
              <div>
                ...
                <div className={`p-6 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
                  <p className={`text-base leading-normal text-justify ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                    Bulan September 2023 saya di panggil kembali oleh Tim UKP Puskesmas Kotabaru untuk membantu mereka dalam pembuatan serta perbaikan SOP & SK Puskesmas sebagai dokumen yang diperlukan dalam proses Akreditasi. namun pada saat itu satus saya hanya sebagai Tenaga Freelance.
                  </p>
                  <p className={`text-base leading-normal mt-3 text-justify ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                    Alhamdulillah pada bulan Oktober 2023 berlangsungnya proses Akreditasi. semua berjalan dengan lancar, mulai dari semua dokumen SOP & SK UKP yang di koreksi oleh Surveyor dan hanya sedikit revisi yang perlu di lakukan. Puskesmas Kotabaru pada saat itu berhasil mendapatkan predikat "Terakreditasi Paripurna".
                  </p>
                </div>
              </div>
              ...
              <div className={`p-6 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Dokumen Diproses</p>
                      <p className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>30+ File</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <Database className="w-5 h-5 text-amber-500" />
                    <div>
                      <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Tingkat Keberhasilan</p>
                      <p className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>98% Lulus</p>
                    </div>
                  </div>
                </div>
              </div>
