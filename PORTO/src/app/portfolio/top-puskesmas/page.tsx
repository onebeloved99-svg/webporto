...
export default function TopPuskesmasDetail() {
  const { isDark, toggleTheme } = useTheme()

  const projectDetails = {
    title: 'Top 10 Puskesmas dengan Penyerapan Anggaran Terbesar',
    subtitle: 'Sistem dashboard untuk monitoring dan evaluasi penyerapan anggaran puskesmas',
    duration: '2021 - 2022',
  ...
  const features = [
    'Dashboard ranking 10 puskesmas terbaik',
    'Visualisasi data penyerapan real-time',
    'Grafik komparasi antar puskesmas',
    'Filter dan sorting berdasarkan parameter',
    'Export laporan ranking dalam PDF/Excel',
    'Historical data tracking',
    'Alert system untuk pencapaian target',
  ...
  const technologies = [
    {
      name: 'Frontend',
      items: ['HTML', 'CSS', 'JavaScript', 'Chart.js', 'DataTables']
    },
    {
      name: 'Backend', 
      items: ['PHP', 'MySQL', 'REST API', 'AJAX']
    },
    {
      name: 'Analytics Tools',
      items: ['Power BI', 'Excel Advanced', 'Google Data Studio', 'Tableau']
    }
  ]
  ...
  const achievements = [
    {
      ...
      title: '100+ Puskesmas Dimonitoring',
      description: 'Memantau kinerja 100+ puskesmas seluruh Indonesia'
    },
    {
      icon: <Zap className="w-5 h-5" />,
      title: 'Real-time Updates',
      description: 'Update data ranking secara real-time setiap hari'
    },
    {
      icon: <Target className="w-5 h-5" />,
      title: '25% Peningkatan Efisiensi',
      description: 'Meningkatkan efisiensi evaluasi kinerja hingga 25%'
    },
    {
      icon: <Award className="w-5 h-5" />,
      title: 'Best Analytics Dashboard',
      description: 'Penghargaan dashboard analitik terbaik tingkat nasional'
    }
  ]

  const challenges = [
    {
      title: 'Data Integration dari Berbagai Sumber',
      description: 'Menggabungkan data dari berbagai sistem dan format (Excel, PDF, database) ke dalam satu platform analitik terpadu dengan proses ETL otomatis.'
    },
    {
      title: 'Algoritma Ranking yang Kompleks',
      description: 'Mengembangkan algoritma perankingan yang adil dan transparan dengan berbagai parameter penilaian seperti penyerapan anggaran, kinerja, dan compliance.'
    },
    {
      title: 'Visualisasi Data yang Komprehensif',
      description: 'Membuat dashboard interaktif yang dapat menampilkan data kompleks dengan visualisasi yang mudah dipahami oleh stakeholder dari berbagai level.'
    }
  ]
              ...
              <div>
                ...
                <div className={`p-6 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
                  <p className={`text-lg leading-relaxed ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                    Top 10 Puskesmas dengan Penyerapan Anggaran Terbesar adalah sistem dashboard analitik yang dikembangkan untuk memantau, mengevaluasi, dan meranking puskesmas berdasarkan kinerja penyerapan anggaran. Sistem ini menyediakan visualisasi data real-time, analisis komparatif, dan tools evaluasi untuk mendukung pengambilan keputusan strategis dalam manajemen kesehatan publik.
                  </p>
                </div>
              </div>
              ...
              <div className={`p-6 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Total Puskesmas</p>
                      <p className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>100+ Units</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <Clock className="w-5 h-5 text-blue-500" />
                    <div>
                      <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Update Frequency</p>
                      <p className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>Daily Real-time</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <Database className="w-5 h-5 text-blue-500" />
                    <div>
                      <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Data Sources</p>
                      <p className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>Multiple Systems</p>
                    </div>
                  </div>
                </div>
              </div>
